...
export function AnswerCard({ answer, onApprove, onReject, onCancel, isSpeaking }: AnswerCardProps) {
    ...
    <div className="glass rounded-lg p-4 space-y-3">
      ...
      <div className="flex gap-2">
        {isSpeaking ? (
          ...
          <>
            <button onClick={onApprove} disabled={!isReady} className="btn-primary flex-1">
              {isReady ? 'Approve to Speak' : 'Approved'}
            </button>
            <button onClick={copyToClipboard} className="btn-secondary" title="Copy text">
            ...
            <button onClick={onReject} className="btn-secondary" title="Reject">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
